...
export class CharactersComponent implements OnInit {
  ...
  URLEps='https://rickandmortyapi.com/api/episode/'
  URLLocs='https://rickandmortyapi.com/api/location/'

  listCharacters: any[] = [];
  constructor(private _characerService: RequestService){}
  ngOnInit() {
    this._characerService.getAllCharacters(this.URLChar).subscribe((data: any)=>{
      this.totalRecords=data.info.count
      console.log(this.totalRecords)
    });
    let array = this.range(0, 9);
    this._characerService.getCharactersRange(this.URLChar, array).subscribe((data: any)=>{
